...
func TestUserClaimJWT(t *testing.T) {
	...
	if jsnerr := json.Unmarshal([]byte(contractDataJSON), &cred); jsnerr != nil {
		tests.FailedWithError(jsnerr, "Should have successfully parsed example.CreateUserSession json")
	}
	tests.Passed("Should have successfully parsed example.CreateUserSession json")
